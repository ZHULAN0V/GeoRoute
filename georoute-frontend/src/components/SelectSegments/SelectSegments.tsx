...
import {
  chooseEndMarkerId,
  chooseStartMarkerId,
} from "../../providers/paths/path-segments-ids-reducer";
...
const SelectSegments = () => {
  ...
  const markers = useMemo(
    () => Object.values(pathObject[currentPathId]?.markers || {}),
    [currentPathId, pathObject],
  );
  ...
  const debouncedMarker = useDebouncedCallback(() => {
    dispatch(
      editMarker({
        ...pathObject[currentPathId]?.markers[markerIds.startMarkerId],
        name: markerName,
      }),
    );
  }, 300);
