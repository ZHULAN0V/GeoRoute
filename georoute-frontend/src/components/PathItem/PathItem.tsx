...
function PathItem(props: IPathItemProps) {
  ...
  const points = useMemo(
    () =>
      Object.values(Object.values(path.variants)[0].path).map(
        (x) => [x.lat, x.lng] as [number, number],
      ),
    [path.variants],
  );
  ...
  return (
    <div
      className={`${styles["path-item"]} ${styles[currentPathId == path.id ? "active" : ""]}`}
      ...
      <div className={styles["right-block"]}>
        ...
        <div className={styles["buttons"]}>
          ...
          >
            ...
            {path.checked && (
              <CheckBoxIcon sx={{ width: 20, height: 20, color: "#212121" }} />
            )}
          </IconButton>
        </div>
      </div>
