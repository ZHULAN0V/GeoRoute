...
function PathVariant(props: IPathVariantProps) {
  ...
  const points = useMemo(
    () =>
      Object.values(
        pathObject[pathVariant.pathId].variants[pathVariant.id].path,
  ...
  const handleSetCheckedPath = (
    ...
    e.stopPropagation();
    dispatch(
      editPathVariant({ ...pathVariant, isVisible: !pathVariant.isVisible }),
    );
  };

  const handleCallMergePath = (
    e: React.MouseEvent<HTMLButtonElement, MouseEvent>,
  ) => {
    e.stopPropagation();
  ...
  return (
    <div
      className={`${styles["path-item"]} ${styles[currentPathVariantId == pathVariant.id ? "active" : ""]}`}
      onClick={handleChosePath}
    >
      <div className={styles["left-block"]}>
        <p className={styles["title"]}>{pathVariant.name}</p>
        <p className={styles["description"]}>
          {calculateRouteDistance(points)} км
        </p>
      </div>
      <div className={styles["right-block"]}>
        <div className={styles["color"]}>
          <div
            ...
            style={{ backgroundColor: pathVariant.color }}
          ></div>
