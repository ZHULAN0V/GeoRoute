...
export const loadPathsFromNamesWithVariantsAndMarkers = async (
  ...
  try {
    const promises = names.map(async (name) => {
      ...
      const markerObjects: { [index: string]: IMarker } = markers.reduce(
        (acc, marker, index) => {
      ...
      const variantsObject: { [index: string]: IPathVariant } =
        routePoints.reduce(
          (acc, routes, index) => {
            const variantId = crypto.randomUUID();

            // todo нет next и prev
            const pointsObject: { [index: string]: IPoint } = routes.reduce(
              (acc, point) => {
