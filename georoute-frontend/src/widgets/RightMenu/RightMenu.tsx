...
function RightMenu() {
  ...
  const [variantName, setVariantName] = useState(
    path?.variants[currentPathVariantId]?.name || "",
  );
  const [variantColor, setVariantColor] = useState(
    path?.variants[currentPathVariantId]?.color || "",
  );

  const filteredVariants = useMemo(() => {
    if (markerIds.startMarkerId != "" && markerIds.endMarkerId != "") {
      return Object.values(pathObject[currentPathId]?.variants || {}).filter(
        (x) =>
          x.startMarkerId == markerIds.startMarkerId &&
          x.endMarkerId == markerIds.endMarkerId,
      );
  ...
  const debounced = useDebouncedCallback(() => {
    ...
    dispatch(
      editPathVariant({
        ...path?.variants[currentPathVariantId],
        name: variantName,
        color: variantColor,
      }),
    );
